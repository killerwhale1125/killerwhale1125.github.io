...
> WAS의 수가 증가함에 따라 부하 분산 효과는 긍정적이었지만, Nginx의 `keepalive_timeout`, `proxy_read_timeout`, `proxy_connect_timeout` 설정에 따른 연결 종료와 WAS의 CPU 또는 메모리 부족으로 인한 비정상 종료가 발생하였습니다. 아무래도 AWS 프리티어는 CPU의 코어를 1개뿐 사용하기 때문에 Nginx 또한 WorkerProcess 가 1개 뿐인지라 VUser를 크게 잡으면 OS 커널에서 대기시간이 길어지기 때문에 아무래도 프로덕션 환경에서의 테스트에서는 좀 다를 것이라고 예상합니다.
