...
**테스트 시나리오**
- 가상 사용자 수(VU)를 10분에 걸쳐 50명으로 설정하여 부하 테스트를 진행.
...
특히, **RDS의 CPU 사용률이 높아지면서 DB에서 병목현상이 발생**하고 있다는 것을 확인할 수 있었습니다.



<br/>

## 커넥션 풀 부족과 RDS 병목 현상 분석
...
> 이 오류는 **DB 커넥션 풀의 최대 크기인 10개가 모두 사용 중일 때 새로운 요청이 대기 상태로 남게 되면서 발생한 문제**입니다. RDS의 CPU 사용량이 높았던 것도 이러한 이유 때문입니다.
...
> **요청 실패 수가 10배 감소**하여 다소 안정적인 모습이 확인되었습니다.
