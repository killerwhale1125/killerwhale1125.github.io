...
테스트 테이블의 정보
![Untitled](https://killerwhale1125.github.io/assets/img/post/index_test.png){: width="800" height="400" .normal}
...
<br/><br/>예제에서는 만약 100만개지만 데이터가 1000만개 아니 1억개라면 어떨까요? 
이와 같이 데이터 `Size가 늘어날 수록 조회 비용이 더욱 커져 성능이 감소`할 것 입니다. <br/><br/>
...
테스트 테이블 정보
![Untitled](https://killerwhale1125.github.io/assets/img/post/table_info_join.png){: width="800" height="400" .normal}
...
옵티마이저 실행계획
1. JOIN 조건인 u.id를 찾기 위하여 BETWEEN '2022-01-01' AND '2024-03-07' 이 조건에 만족하는 게시물을 100만개 중 21만개를 조회한다.
2. 21만개의 게시물과 user 와 JOIN하여 User0000046인 것을 필터링하고 평균 13.7e-6 개의 데이터를 조회했다.
3. Nested loop inner join 으로 Users와 Posts 테이블을 조인하며 3개의 데이터를 필터링하였다.
